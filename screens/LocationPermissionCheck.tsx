...
const App: FC = ({navigation}: any) => {
  ...
  useEffect(() => {
    ...
    UserGeolocationService.instance
      ...
      .catch(() => {
        setForegroundLocationPermission("false");
      }); // if it fails set as false
  }, []);

  useEffect(() => {
    console.log(
      "Foreground Location Permission: " + foregroundLocationPermission,
    );
    if (
  ...
  if (foregroundLocationPermission === "false") {
    return (
      <SafeAreaView>
        <Text>Instructions on how to change foreground to allow</Text>
      </SafeAreaView>
    );
  } else if (backgroundLocationPermission === "false") {
    return (
      <SafeAreaView>
        <Text>Instructions on how to change background to allow</Text>
      </SafeAreaView>
    );
  } else {
    // waiting for checks still
    return (
      <SafeAreaView>
