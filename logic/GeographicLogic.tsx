...
export const diffBetweenLatlong = function (
  lat1: number,
  lon1: number,
  lat2: number,
  lon2: number,
...
const intersects = (
  ...
  if (
    (point1.long === point2.long && point1.lat === point2.lat) ||
    (point3.long === point4.long && point3.lat === point4.lat)
  ) {
    throw "no intersection found";
  }

  const denominator =
    (point4.lat - point3.lat) * (point2.long - point1.long) -
    (point4.long - point3.long) * (point2.lat - point1.lat);
  ...
  let ua =
    ((point4.long - point3.long) * (point1.lat - point3.lat) -
      (point4.lat - point3.lat) * (point1.long - point3.long)) /
    denominator;
  let ub =
    ((point2.long - point1.long) * (point1.lat - point3.lat) -
      (point2.lat - point1.lat) * (point1.long - point3.long)) /
    denominator;
...
export const calcIntersections = (
  userLocation: Point,
  path: Point[],
  rangeRadius: number,
): boolean => {
  for (let i = 0; i < path.length - 1; i++) {
    ...
    const endPoint = new Point(
      userLocation.long +
        perpendicular.x * rangeRadius * LAT_LONG_PER_M_AT_EQUATOR,
      userLocation.lat +
        perpendicular.y * rangeRadius * LAT_LONG_PER_M_AT_EQUATOR,
    ); // range radius will be huge here and not represent meters
    ...
    try {
      const interSectionPoint = intersects(
        startPoint,
        endPoint,
        path[i],
        path[i + 1],
      ...
      if (interSectionPoint.distanceTo(userLocation) <= rangeRadius) {
        // if the user is close enough to the line
