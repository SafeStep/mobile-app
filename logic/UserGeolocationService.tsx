...
class UserGeolocationServiceState {
  ...
  incrimentOnTracks(): void {
    this.consecOnTracks++;
    if (
      this.consecOnTracks >= UserGeolocationService.CONSEC_ON_TRACK_TO_START
    ) {
      this.pathJoined = true;
      console.log(
        "Users has joined the path, on leave alarm will be triggered",
      );
    }
  }
...
export class UserGeolocationService {
  ...
  private constructor(state?: UserGeolocationServiceState) {
    ...
    if (
      TaskManager.isTaskDefined(UserGeolocationService.BACKGROUND_TASK_NAME)
    ) {
      this.stopBackgroundWatch();
    }
  }

  static build(): UserGeolocationService {
    // creates a new UserGeoLocation service
    if (UserGeolocationService.instance) {
  ...
  isUserOnPath(): boolean {
    ...
    const userPoint = new Point(
      this.cachedLocation.long,
      this.cachedLocation.lat,
    );
    return calcIntersections(
      userPoint,
      this.state.getTrackedPath(),
      UserGeolocationService.RANGE_RADIUS,
    );
  }
  ...
  async startPathTracking(path: number[][]): Promise<void> {
    ...
    Location.startLocationUpdatesAsync(
      UserGeolocationService.BACKGROUND_TASK_NAME,
      {
        foregroundService: {
        ...
        timeInterval:
          UserGeolocationService.BACKGROUND_LOCATION_INTERVAL * 1000,
        distanceInterval: 30,
  ...
  startForegroundWatch(): void {
    if (this.watchRemove != defaultWatchRemove) {
      console.error("Watch already running!");
    } // stop the creation of two watches
    ...
    Location.watchPositionAsync(
      {
        ...
        timeInterval:
          UserGeolocationService.FOREGROUND_LOCATION_INTERVAL * 1000,
      },
      location => {
        _this.cachedLocation = {
      ...
      .then(removeCallback => {
        this.watchRemove = removeCallback.remove;
      })
      .catch(err => {
        console.error(err);
      });
  ...
  stopBackgroundWatch(): void {
    ...
    if (
      ...
      // if the task exists and has started
  ...
  getLocation(): Promise<PhysicalLocation> {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (permissionStatus === Location.PermissionStatus.DENIED) {
        ...
        Location.getCurrentPositionAsync({accuracy: LocationAccuracy.Highest})
          .then(location => {
            // TODO test accuracy of this
            let newLocation = {
              lat: location.coords.latitude,
              long: location.coords.longitude,
              title: "Current Location",
...
TaskManager.defineTask(
  ...
  ({data, error}: any) => {
    ...
    const location = {
      ...
      title: "Current Location",
    } as PhysicalLocation; // TODO check if this is the most recent or the oldest value
